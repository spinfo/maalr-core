...
public class LocalDB {
	...
	private static Logger logger = Logger.getLogger(LocalDB.class);
	...
	public static TranslationMap getEditorTranslation(String locale) {
		...
		if (time < System.currentTimeMillis() - UPDATE_INTERVAL) {
			Logger.getLogger(LocalDB.class).info(
					"Editor translations outdated.");
			return null;
		}
		...
		Logger.getLogger(LocalDB.class).info(
				"Parsed editor translations for locale " + locale);
		return toReturn;
	}
	...
	public static void setEditorTranslation(String locale, TranslationMap map) {
		...
		logger.info("Stored translations for locale " + locale);
	}
