...
public class SocialConfig {
	...
	private static final String TWITTER = "twitter", FACEBOOK="facebook", GOOGLE="google", LINKEDIN="linkedin";
	...
	public ConnectionFactoryLocator connectionFactoryLocator() {
		...
		// add linkedIn
		if(environment.getAppConfiguration().getSocialClientKey(LINKEDIN) != null) {
			logger.info("Initializing google authentication service...");
			OAuth2ConnectionFactory<LinkedIn> connectionFactory = new LinkedInConnectionFactory(
					environment.getAppConfiguration().getSocialClientKey(LINKEDIN),
					environment.getAppConfiguration().getSocialClientSecret(LINKEDIN));
			registry.addConnectionFactory(connectionFactory);
		} else {
			logger.info("NOT initializing linkedIn authentication service...");
		}
